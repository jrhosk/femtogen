...
class FemtoGen:
    ...
    def calculate_kinematics(self, phi: float)->'None':
        ...
        :param phi: scattering angle relative to the x-y plane.
        :return: None
        '''
        ...
        self.xi = self.kinematics.xbj * (
                (1 + (self.kinematics.t / (2 * self.kinematics.Q2))) / (2 - self.kinematics.xbj + ((self.kinematics.xbj * self.kinematics.t) / self.kinematics.Q2)))
        ...
        self.cost = -(1 / math.sqrt(1 + math.pow(self.gamma, 2))) * (1 + (.5 * math.pow(self.gamma, 2)) * (
                (1 + (self.kinematics.t / self.kinematics.Q2)) / (1 + ((self.kinematics.xbj * self.kinematics.t) / (self.kinematics.Q2)))))
        ...
        self.GAMMA = math.pow(constants.alpha, 3) / (
                16 * self.kinematics.xbj * math.pow(constants.pi, 2) * math.pow((self.s - math.pow(self.proton_mass, 2)),2) * math.sqrt(1 + math.pow(self.gamma, 2)))
    ...
    def _calculate_dvcs(self, phi: float)->'float, float, float':
        ...
        A = 4 * (1 - math.pow(self.xi, 2)) * (
                math.pow(self.kinematics.ReH, 2) + math.pow(self.kinematics.ImH, 2) + math.pow(self.kinematics.ReHt, 2) + math.pow(self.kinematics.ImHt,
                                                                                                           2))
    ...
    def generate_cross_section(self, phi: 'np.array', type='full', error=None) -> 'iterator':
        ...
        for p in tqdm(phi):
            ...
            B_term = -B * conversion * (self.GAMMA / (-self.kinematics.t * self.kinematics.Q2)) * (self.F1 + self.F2) * (
                    self.kinematics.ReH + self.kinematics.ReE)
            C_term = -C * conversion * (self.GAMMA / (-self.kinematics.t * self.kinematics.Q2)) * (self.F1 + self.F2) * self.kinematics.ReHt
    ...
    def read_data_file(self, file: str)->'list':
        ...
        :param file: comma seperated text file containing kinematic varilable information
    ...
    def set_kinematics(self, xbj=0., t=0., Q2=0., k_0=0., ReH=0., ReE=0., ImH=0., ImE=0., ReHt=0., ReEt=0., ImHt=0., ImEt=0., **kwargs)->'pystruct':
        ...
        :param t: momentum transfer of proton
        :param Q2: momentum transfer
